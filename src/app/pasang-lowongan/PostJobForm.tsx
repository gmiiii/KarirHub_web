...
export function PostJobForm() {
  ...
  return (
    <form
      className="grid gap-lg lg:grid-cols-[1fr_300px]"
      onSubmit={(e) => {
        e.preventDefault();
        toast('Lowongan berhasil ditayangkan');
        router.push('/dashboard/rekruter');
      }}
    >
        ...
        <div className="rounded-xl border border-outline-variant bg-surface-container-lowest p-lg lg:sticky lg:top-24">
          ...
          <div className="mt-md rounded-lg bg-surface-container-low p-md">
            <p className="flex items-center gap-2 text-caption text-on-surface-variant">
              <Icon name="info" size={16} className="text-primary" /> Lowongan ditinjau maksimal 1x24 jam sebelum tayang.
            </p>
          </div>
          <div className="mt-lg space-y-sm">
            <Button type="submit" fullWidth icon="publish">Tayangkan Lowongan</Button>
            <Button type="button" variant="ghost" fullWidth icon="visibility" onClick={() => toast('Membuka pratinjau lowongan', 'info')}>
              Pratinjau
            </Button>
            <Button type="button" variant="secondary" fullWidth icon="save" onClick={() => toast('Draf lowongan disimpan')}>
              Simpan Draf
            </Button>
          </div>
        </div>
